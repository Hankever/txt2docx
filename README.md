...
- [安装包](https://github.com/Hankever/txt2docx/releases/latest)
...
- 支持 `txt` 转 `docx`、`epub` 转 `docx`、`docx` 转 `txt`、`epub` 转 `txt`
- EPUB 转 DOCX 会按章节顺序提取正文，并嵌入 EPUB 内的常见图片资源（PNG/JPEG/GIF/BMP/TIFF）
- EPUB 转 TXT 会按章节顺序提取纯文本正文（忽略图片）
- 自动识别常见文本编码，也可手动指定编码
- 可设置字体、字号、页边距
...
```bash
java -jar target/txt2docx.jar --mode epub2txt --input ./epub --output ./txt --recursive
```
